...
void *ion_map_kernel(struct ion_client *client, struct ion_handle *handle)
{
	...
	objp = ion_handle_kmap_get(handle);
	ion_handle_put(handle);
	return objp;
}
...
struct ion_handle *ion_alloc(struct ion_client *client, size_t len,
			     ...
			     unsigned int flags)
{
	struct ion_heap *heap, *prev_heap = NULL;
	...
	while (1) {
		...
		read_lock(&dev->heap_lock);
		plist_for_each_entry(heap, &dev->heaps, node) {
			if (heap != prev_heap && BIT(heap->id) & heap_id_mask) {
				found = true;
				break;
			}
		}
		read_unlock(&dev->heap_lock);
	...
	if (ion_handle_add(client, handle, false))
		goto put_handle;
...
{
	/* Do a double put to undo ion_handle_validate_get's reference */
	if (ion_handle_validate_get(client, handle)) {
		ion_handle_put(handle);
		ion_handle_put(handle);
	}
...
struct ion_handle *ion_import_dma_buf(struct ion_client *client, int fd)
{
	...
	if (IS_ERR(handle)) {
		...
		ret = ion_handle_add(client, handle, true);
		if (ret)
			goto put_handle;
	}
...
static long ion_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case ION_IOC_FREE:
		...
		/* Do a double put to undo ion_handle_get_by_id's reference */
		ion_handle_put(handle);
		ion_handle_put(handle);
		break;
	case ION_IOC_SHARE:
...
void ion_device_add_heap(struct ion_device *dev, struct ion_heap *heap)
{
	...
	write_lock(&dev->heap_lock);
	plist_add(&heap->node, &dev->heaps);
	write_unlock(&dev->heap_lock);
}
